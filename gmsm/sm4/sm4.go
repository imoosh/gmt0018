...
//20220719 添加的sm4 gcm测试函数
//功能已验证正常，但是很难支持三段式加解密，暂时放弃
func Sm4GcmEnc() {
	plain, _ := hex.DecodeString("0000000000000000000000000000000000")
	...
	cipherText := sm4gcm.Seal(nil, iv, plain, nil)
...
func Sm4GcmDec() {
	cipherText, _ := hex.DecodeString("7de2aa7f1110188218063be1bfeb6d89b851b5f39493752be508f1bb4482c557")
	key, _ := hex.DecodeString("00000000000000000000000000000000")
	iv, _ := hex.DecodeString("000000000000000000000000")
	...
	plain, _ := sm4gcm.Open(nil, iv, cipherText, nil)

	// encode as base64 string
	encoded := hex.EncodeToString(plain)
	fmt.Printf("%s\n", encoded)
}
